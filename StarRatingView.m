...
- (id)initWithFrame:(CGRect)frame andRating:(int)rating withLabel:(BOOL)label animated:(BOOL)animated
{
    ...
    if (self) {
        ...
        if (self.userRating >=20.0f) {
            ...
            [tintLayer setBackgroundColor:[UIColor redColor].CGColor];
        }
...
{
    if (self.userRating < 20.0f) {
        [self.tintLayer setBackgroundColor:[UIColor redColor].CGColor];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (CGRectContainsPoint(self.bounds, [[[touches allObjects]lastObject] locationInView:self])) {
        ...
        [self ratingDidChange];
        singleTonInstance.userRatingStr =  [NSString stringWithFormat:@"%d",self.rating];
        if (self.label) {
            ...
            NSLog(@"ratings from starrating class %@",singleTonInstance.userRatingStr);
